...
def main(cfg: DictConfig):
    ...
    comm = utils.MPI_COMM()
    comm.init(cfg, print_hello=True)
    ...
    if cfg.train.repeatability:
        # See SDL workshop SmartSim example for how to use these
    ...
    if not cfg.database.launch:
         data, mesh_nodes = utils.load_data(cfg)
    ...
    if (comm.rank == 0):
        print(f"Running on device: {cfg.train.device} \n")
        sys.stdout.flush()
