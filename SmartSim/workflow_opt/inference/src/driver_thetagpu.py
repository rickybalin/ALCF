...
def launch_coDB(cfg, nodelist, nNodes, hostfile):
    # Print nodelist
    ...
    runArgs = {'hostfile': hostfile,
               'n': cfg.run_args.simprocs, 
               'npernode': cfg.run_args.simprocs_pn,
               'x': 'PATH'
              }
    run_settings = MpirunSettings(
                   'python',
                   exe_args=exe_args,
                   run_args=runArgs,
                   env_vars=None
...
def launch_clDB(cfg, nodelist, nNodes):
    ...
    PORT = cfg.experiment.port
    exp = Experiment(cfg.experiment.name, launcher=cfg.experiment.launcher)
    runArgs = {"n": 1, "npernode": cfg.run_args.cores_pn}
    ...
    run_settings = MpirunSettings('python',
                  exe_args=exe_args,
                  run_args=None
                  )
    run_settings.set_tasks(cfg.run_args.simprocs)
