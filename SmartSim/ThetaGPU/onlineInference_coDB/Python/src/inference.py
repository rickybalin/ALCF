...
def main():
    ...
    if (rank==0 or rank==24):
        client.set_model_from_file('model', './model_jit.pt', 'TORCH', device='GPU')
        print(f'Uploaded model to database from rank {rank}')
    comm.Barrier()
    ...
    if (rank==0):
        fid0 = open('./predictions_node1.dat', 'w')
    if (rank==24):
        fid24 = open('./predictions_node2.dat', 'w')
    ...
    for its in range(numts):
        ...
        if (rank==0 or rank==24):
            ...
            for i in range(nSamples):
                if (rank==0):
                    fid0.write(f'{inputs[i,0]:.6e} {predictions[i,0]:.6e} {truth[i,0]:.6e}\n')
                if (rank==24):
                    fid24.write(f'{inputs[i,0]:.6e} {predictions[i,0]:.6e} {truth[i,0]:.6e}\n')
