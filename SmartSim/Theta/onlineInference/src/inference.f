...
c     Load the model on the database
      if (myrank.eq.0) then
         call client%set_model_from_file("model", "model_jit.pt",
     &                                   "TORCH", "CPU")
...
c     Emulate integration of PDEs with a do loop
     ...
     &            shape(inf_data)
         call client%put_tensor(trim(inf_key), inf_data, 
     &                          shape(inf_data))
         call MPI_Barrier(MPI_COMM_WORLD,ierr)
         if (myrank.eq.0) write(*,*) 'Finished sending inference data'

         ! Send the predictions only on the first iteration to create a tensor with the correct keys
!         if (its.eq.1) then
!            pred_data = truth_data
!            call client%put_tensor(trim(pred_key), pred_data,
!     &                             shape(pred_data))
!         endif
!         call MPI_Barrier(MPI_COMM_WORLD,ierr)
!         if (myrank.eq.0) write(*,*) 'Finished sending prediction data',
!     &                               ' at first time step'

         ! Evaluate the model on the database
         inputs(1) = inf_key
         outputs(1) = pred_key
         call client%run_model('model', inputs, outputs)
         call MPI_Barrier(MPI_COMM_WORLD,ierr)
         if (myrank.eq.0) write(*,*) 'Finished evaluating model'

         ! Retreive the predictions
         call client%unpack_tensor(trim(pred_key), pred_data,
     &                             shape(pred_data))
         call MPI_Barrier(MPI_COMM_WORLD,ierr)
